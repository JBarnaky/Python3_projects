...
def generate_frames():
    ...
    with mss.mss() as sct:
        ...
        while True:
            ...
            sleep_time = max(0, 1 / FRAMES_PER_SECOND - processing_time)
            time.sleep(sleep_time) if sleep_time > 0 else logger.warning(
                f"Frame processing took longer than 1/{FRAMES_PER_SECOND} seconds: {processing_time}s"
            )
